...
- [x] 階段十二：衛教單張列印品質（2026-08-09，v1.1）—— `@page` 由 `size:A4` 改為
      A4∩Letter 交集 `210mm 279.4mm`；並修正比一頁還高的卡片（居家檢查表 287.4mm）
      掛 `page-break-inside:avoid` 反而讓斷點失控的問題
...
- **會進影印機的頁面（衛教單張），`@page` 一律用 A4∩Letter 交集 `210mm 279.4mm`**（寬取 A4、高取 Letter）：
  醫院影印機紙匣有時放的是 Letter，Letter 比 A4 寬 5.9mm 但**短 17.6mm**——寫 `size:A4` 會被 Letter 裁掉底部，
  寫 `size:Letter` 又會在 A4 上裁掉左右，只有取交集兩邊都安全。
  真正決定會不會被裁的是**內容實際結束在哪**，排完要實際量：
  - 可用高度＝279.4 − 上下邊界；**瀏覽器的「頁首及頁尾」開著會再吃掉上下各約 12mm**，
    以 `margin:10mm 15mm 12mm` 計，開著時每頁只剩約 **233mm**。印之前要提醒對方關掉。
  - **任何掛 `page-break-inside:avoid` 的區塊都不可高於可用頁高**，否則瀏覽器只能忽略它、斷點反而不可控
    （2026-08-09 踩到：居家檢查卡 287.4mm）。長內容改成允許跨頁，只保護真正不該拆開的小單位。
  - 要省空間的順序：**先動間距 → 再動行高（1.5 是下限）→ 絕不動字級**（讀者常是 50 歲以上）。頁數不是問題，被裁切才是。
